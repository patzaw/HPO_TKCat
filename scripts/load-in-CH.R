source("~/opt/KM_patrice.godard.R")
# library(TKCat)
# .tkcon <- chTKCat(port = 9113, user = "patrice.godard@angelinipharma.com")
library(here)

fdb <- read_fileMDB(here())
rn <- db_info(fdb)$name

chdb <- get_MDB(.tkcon, rn, check=FALSE)
stopifnot(db_info(fdb)$name==db_info(chdb)$name)

# rdkmspec <- read_fileMDB(here(sprintf("RDKM/RDKM specification for %s", rn)))
tst <-  max(c(
   # file.info(data_files(rdkmspec)$dataFiles)$mtime,
   file.info(data_files(fdb)$dataFiles)$mtime
))
fdb <- c(fdb)#, rdkmspec)
db_info(fdb)$timestamp <- tst

## Full upload ----

# message("In memo")
# message(Sys.time())
# mdb <- as_memoMDB(fdb, check=FALSE)
# # db_reconnect(.tkcon)
# message("In CH")
# message(Sys.time())
# chdb <- as_chMDB(mdb, .tkcon, timestamp=db_info(mdb)$timestamp, by=10^7)
# message(Sys.time())

## Update ----

# newdb <- list(
#    ori=select(
#       chdb, -all_of(intersect(names(chdb), c(names(rdkmspec))))
#    ),
#    part=select(fdb, c(names(rdkmspec)))
# ) %>% set_names(c(rn, "part"))
# db_info(newdb$part)$name <- "part"
# newdb <- metaMDB(
#    MDBs=newdb, relationalTables=NULL,
#    dataModel=data_model(fdb),
#    dbInfo=db_info(fdb),
#    check=FALSE
# )
# chdb <- as_chMDB(newdb, .tkcon, timestamp=db_info(newdb)$timestamp, by=10^7)
