-- MySQL Script generated by MySQL Workbench
-- Thu Nov 22 12:09:19 2018
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema HPO
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema HPO
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `HPO` DEFAULT CHARACTER SET utf8 ;
USE `HPO` ;

-- -----------------------------------------------------
-- Table `HPO`.`HPO_hp`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HPO`.`HPO_hp` (
  `id` VARCHAR(45) NOT NULL,
  `name` VARCHAR(45) NOT NULL,
  `description` VARCHAR(45) NULL,
  `level` INT NOT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `HPO`.`HPO_altId`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HPO`.`HPO_altId` (
  `alt` VARCHAR(45) NOT NULL,
  `id` VARCHAR(45) NOT NULL,
  INDEX `fk_HPO_altId_HPO_hp1_idx` (`id` ASC) VISIBLE,
  CONSTRAINT `fk_HPO_altId_HPO_hp1`
    FOREIGN KEY (`id`)
    REFERENCES `HPO`.`HPO_hp` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `HPO`.`HPO_sourceFiles`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HPO`.`HPO_sourceFiles` (
  `url` VARCHAR(45) NOT NULL,
  `current` DATE NOT NULL,
  PRIMARY KEY (`url`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `HPO`.`HPO_diseases`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HPO`.`HPO_diseases` (
  `db` VARCHAR(45) NOT NULL,
  `id` VARCHAR(45) NOT NULL,
  `label` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`db`, `id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `HPO`.`HPO_diseaseHP`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HPO`.`HPO_diseaseHP` (
  `db` VARCHAR(45) NOT NULL,
  `id` VARCHAR(45) NOT NULL,
  `hp` VARCHAR(45) NOT NULL,
  INDEX `fk_HPO_diseaseHp_HPO_hp1_idx` (`hp` ASC) VISIBLE,
  INDEX `fk_HPO_diseaseHp_HPO_diseases1_idx` (`db` ASC, `id` ASC) VISIBLE,
  CONSTRAINT `fk_HPO_diseaseHp_HPO_hp1`
    FOREIGN KEY (`hp`)
    REFERENCES `HPO`.`HPO_hp` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_HPO_diseaseHp_HPO_diseases1`
    FOREIGN KEY (`db` , `id`)
    REFERENCES `HPO`.`HPO_diseases` (`db` , `id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `HPO`.`HPO_diseaseSynonyms`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HPO`.`HPO_diseaseSynonyms` (
  `db` VARCHAR(45) NOT NULL,
  `id` VARCHAR(45) NOT NULL,
  `synonym` VARCHAR(45) NOT NULL,
  `preferred` TINYINT NOT NULL,
  INDEX `fk_HPO_diseaseSynonyms_HPO_diseases1_idx` (`db` ASC, `id` ASC) VISIBLE,
  CONSTRAINT `fk_HPO_diseaseSynonyms_HPO_diseases1`
    FOREIGN KEY (`db` , `id`)
    REFERENCES `HPO`.`HPO_diseases` (`db` , `id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `HPO`.`HPO_parents`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HPO`.`HPO_parents` (
  `id` VARCHAR(45) NOT NULL,
  `parent` VARCHAR(45) NOT NULL,
  INDEX `fk_HPO_altId_HPO_hp1_idx` (`id` ASC) VISIBLE,
  INDEX `parent_idx` (`parent` ASC) VISIBLE,
  CONSTRAINT `fk_HPO_altId_HPO_hp10`
    FOREIGN KEY (`id`)
    REFERENCES `HPO`.`HPO_hp` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `parent`
    FOREIGN KEY (`parent`)
    REFERENCES `HPO`.`HPO_hp` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `HPO`.`HPO_descendants`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HPO`.`HPO_descendants` (
  `id` VARCHAR(45) NOT NULL,
  `descendant` VARCHAR(45) NOT NULL,
  INDEX `fk_HPO_altId_HPO_hp1_idx` (`id` ASC) VISIBLE,
  INDEX `parent_idx` (`descendant` ASC) VISIBLE,
  CONSTRAINT `fk_HPO_altId_HPO_hp100`
    FOREIGN KEY (`id`)
    REFERENCES `HPO`.`HPO_hp` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `descendant`
    FOREIGN KEY (`descendant`)
    REFERENCES `HPO`.`HPO_hp` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `HPO`.`HPO_synonym`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HPO`.`HPO_synonym` (
  `id` VARCHAR(45) NOT NULL,
  `syn` VARCHAR(100) NULL,
  CONSTRAINT `id`
    FOREIGN KEY (`id`)
    REFERENCES `HPO`.`HPO_hp` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
